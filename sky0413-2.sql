...
select * from student;
 ...
 (
  ...
  INSERT INTO SCORES VALUES (9, '국어',  805, 5); -- ORA-02290: 체크 제약조건(SKY.SYS_C008385)이 위배되었습니다
  ...
  1. INSERT(추가) - 줄(DATA) 추가
     ...
     3) INSERT문 여러개 한번에 실행, 여러줄 추가 : 신규 문법
        CREATE TABLE EX_SKY
        (
  ...
  3. UPDATE -- 줄에 변화는없고 칸에 있는 정보만 수정함
  ...
  --------------------------------------------------------
  DATA 제거
  1. DROP TABLE SCORES; -- 구조(테이블), DATA 모두삭제. 복구 불가능
  
  2. TRUNCATE TABLE SCORES; -- 구조남기고 DATA만 삭제. 속도 빠름
  
  3. DELETE FROM SCORES; -- 구조남기고 DATA만 삭제. 속도 느림
  ...
  외래키 관계에서 자식테이블의 DATA를 지우고 부모 테이블의 DATA를 삭제하면 된다
 ...
 -- 학번, 이름, 점수(국어)
 SELECT STUDENT.STID,STNAME,SUBJECT
 FROM   STUDENT, SCORES
 WHERE  STUDENT.STID = SCORES.STID
 AND    SUBJECT = '국어'
 ;
  
 -- 학번, 이름, 총점, 평균
 SELECT STUDENT.STID,STNAME,ROUND(AVG(SCORE),2),SUM(SCORE)
 FROM   STUDENT, SCORES
 WHERE  STUDENT.STID = SCORES.STID
 GROUP BY STUDENT.STID,STNAME
 ;
